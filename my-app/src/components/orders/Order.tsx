...
const Order = () => {
    ...
    async function fetchOrder(){
        setLoading(true);
        const response = await axiosInstance(`http://127.0.0.1:8000/orders/${orderId}/`);
    ...
    return (
        <>
            {!loading && orderData && orderData.order && orderData.order.created_at && <div className='flex flex-col w-[90%] p-16 m-auto'>
                <div className='flex justify-between'>
                    <div className='flex flex-col gap-4'>
                        <h3 className='text-3xl font-semibold'>{`Order ID: #${orderData.order.order_id}`}</h3>
                        <p className='text-xl text-gray-400'>{orderData.order.created_at ? formatDate(orderData.order.created_at.split('T')[0]) : ''}</p>
                    </div>
                ...
                {/* <p className='mt-4 mb-12 font-semibold text-green-700 bg-green-300 w-fit rounded-3xl px-2 py-1'>{orderData.status}</p> */}
                ...
                <OrderProducts orderItems={orderItems}/>
            </div>
            }
            {loading && <p>Loading...</p>}
        </>
